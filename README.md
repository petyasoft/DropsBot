...
# Software for DropsBot telegram Bot
...
- Follow [Telegram channel](https://t.me/petyasofts)
...
3. Configure the application:
   ...
   - If you want to use a proxy, set `USE_PROXY` in `config.py` to `True`, otherwise set it to `False`:
     ...
     ```
   - If you want to enable proxy checking for each session, set the `CHECK_PROXY` parameter in `config.py` to `True`, otherwise set it to `False`:
     ```python
...
Thank you for using DropsBot Telegram Bot! Don't forget to follow our [Telegram channel](https://t.me/petyasofts) for updates.
